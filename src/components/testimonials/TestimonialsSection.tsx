...
const TestimonialsSection = () => {
  ...
  return <section ref={sectionRef} className="mb-8 sm:mb-12 relative">
      ...
      <div className="relative">
        ...
        <Button variant="outline" size="icon" className="hidden lg:flex absolute left-0 top-1/2 -translate-y-1/2 -translate-x-12 z-10 h-12 w-12 rounded-full bg-background/80 backdrop-blur-sm border-2 hover:bg-background hover:scale-110 transition-all duration-300 disabled:opacity-30 disabled:hover:scale-100 shadow-lg" onClick={scrollPrev} disabled={!canScrollPrev} aria-label="Previous testimonials">
          <ChevronLeft className="h-5 w-5" />
        </Button>

        <Button variant="outline" size="icon" className="hidden lg:flex absolute right-0 top-1/2 -translate-y-1/2 translate-x-12 z-10 h-12 w-12 rounded-full bg-background/80 backdrop-blur-sm border-2 hover:bg-background hover:scale-110 transition-all duration-300 disabled:opacity-30 disabled:hover:scale-100 shadow-lg" onClick={scrollNext} disabled={!canScrollNext} aria-label="Next testimonials">
        ...
        <div className="overflow-hidden" ref={emblaRef}>
          <div className="flex gap-4 md:gap-6">
            {testimonials.map((testimonial, index) => <div key={testimonial.id} className={`flex-[0_0_100%] min-w-0 sm:flex-[0_0_calc(50%-8px)] lg:flex-[0_0_calc(33.333%-16px)] transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`} style={{
            transitionDelay: `${index * 100}ms`
                ...
                <Card className="relative overflow-hidden border-border/40 bg-card/80 backdrop-blur-xl h-full">
                  <div className="relative p-5 sm:p-6 md:p-7 space-y-4 sm:space-y-5 flex flex-col h-full">
                    ...
                    }).map((_, i) => <Star key={i} className="h-4 w-4 fill-warning text-warning" />)}
                      </div>
                    ...
                    <div className="relative flex-1">
                      <svg className="absolute -top-1 -left-1 sm:-top-2 sm:-left-2 w-6 h-6 sm:w-8 sm:h-8 text-primary/20" fill="currentColor" viewBox="0 0 24 24">
                    ...
                    <div className="flex items-center gap-3 pt-4 sm:pt-5 border-t border-border/40">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          {testimonial.verified && <BadgeCheck className="h-4 w-4 text-primary fill-primary/20 flex-shrink-0" />}
                        </div>
      ...
      <div className="flex justify-center items-center gap-2 mt-8 sm:mt-10">
        {testimonials.map((_, index) => <button key={index} onClick={() => scrollTo(index)} className={`transition-all duration-300 rounded-full ${index === selectedIndex ? 'w-8 h-2 bg-primary' : 'w-2 h-2 bg-muted hover:bg-primary/50'}`} aria-label={`Go to testimonial ${index + 1}`} aria-current={index === selectedIndex ? 'true' : 'false'} />)}
      </div>

      {/* Mobile Navigation Buttons */}
      <div className="flex lg:hidden justify-center gap-3 mt-6">
        <Button variant="outline" size="sm" className="h-10 w-10 rounded-full bg-background/80 backdrop-blur-sm border-2 hover:bg-background hover:scale-110 transition-all duration-300 disabled:opacity-30 disabled:hover:scale-100" onClick={scrollPrev} disabled={!canScrollPrev} aria-label="Previous testimonials">
          <ChevronLeft className="h-4 w-4" />
        </Button>

        <Button variant="outline" size="sm" className="h-10 w-10 rounded-full bg-background/80 backdrop-blur-sm border-2 hover:bg-background hover:scale-110 transition-all duration-300 disabled:opacity-30 disabled:hover:scale-100" onClick={scrollNext} disabled={!canScrollNext} aria-label="Next testimonials">
      ...
      <div className="mt-12 sm:mt-16 relative animate-fade-in">
        ...
        <div className="absolute inset-0 -z-10 overflow-hidden pointer-events-none">
          <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[300px] bg-gradient-to-r from-primary/10 via-primary/5 to-accent/10 rounded-full blur-[120px] animate-pulse" style={{
